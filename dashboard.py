...
from services.real_data_engineering import (
    ...
    engineer_drought_features, 
    compute_national_batch_features
)

# Configure Logging
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(name)s: %(message)s")
logger = logging.getLogger("MEHWS_Engine")
...
try:
    ...
    logger.warning(f"Could not apply SimpleImputer patch: {e}")
# ---------------------------------------------------------
...
def load_ml_pipelines():
    """Robustly locates and loads pre-trained scikit-learn ensemble pipelines."""
    possible_dirs = [
        BASE_DIR,
        BASE_DIR / "artifacts",
        BASE_DIR.parent,
    ...
    if flood_path and flood_path.exists():
        try:
            ...
            logger.info(f"Successfully loaded flood model from {flood_path}")
        except Exception as e:
    ...
    if drought_path and drought_path.exists():
        try:
            ...
            logger.info(f"Successfully loaded drought model from {drought_path}")
        except Exception as e:
...
def load_spatial_nodes():
    ...
    if 'lat' not in df.columns or 'lon' not in df.columns:
        raise ValueError("Latitude ('lat') or longitude ('lon') columns missing from spatial CSV.")
...
def compute_national_hazard_map(df_nodes):
    """Computes vectorized national batch hazard inference across all spatial nodes simultaneously."""
...
with tab_map_flood:
    ...
    st.markdown(f"""
    **Active View:** Displaying **{map_flood_horizon}** nationwide batch inference (Target Zone synced with live API forecast: **{selected_flood_score*100:.1f}%**).  
...
with tab_map_drought:
    ...
    st.markdown(f"""
    **Active View:** Displaying **{horizon_label}** nationwide batch inference (Target Zone synced with live API forecast: **{selected_drought_score*100:.1f}%**).  
    **GIS Legend:** 🔴 **High Risk (>50%)** | 🟡 **Moderate Risk (20-50%)** | 🟢 **Low Risk (<20%)**
    """)
