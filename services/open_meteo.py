...
def fetch_seasonal_climate(lat: float, lon: float, days: int = 180):
    """Fetches and extends live weather data to construct real-time seasonal outlooks."""
    url = f"https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&daily=precipitation_sum,temperature_2m_max&timezone=Africa%2FNairobi&forecast_days=16"
    try:
        response = requests.get(url, timeout=10)
        if response.status_code == 200:
            data = response.json()
